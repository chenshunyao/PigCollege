 ...
 */
public class SubjectActivity extends BaseActivity implements View.OnClickListener,AdapterView.OnItemClickListener,HotViewPager.BannerViewClick,HotViewPager.bannerDataCallback{
...
//        setListViewHeightBasedOnChildren(mListView);
        ...
        mAnimation = AnimationUtils.loadAnimation(SubjectActivity.this, R.anim.nn);
            ...
            case CodeUtil.CmdCode.MsgTypeCode.MSG_GET_SUBJECT_ARTICLE_LIST:{
                ...
                if (Utils.getRequestStatus(response) == 1) {
                    ...
                    for (int i = 0; i < contents.size(); i++) {
                        ...
                        article.setImage(content.getArtphoto());
          ...
          case CodeUtil.CmdCode.MsgTypeCode.MSG_CLASS_COURSE_FAV_OPT:{
                ...
                if(Utils.getRequestStatus(response1) == 1) {
                    ...
                    if (httpCommitQaResponse.getResponse().getSucceed() == 1) {
                        ToastUtil.showToast(SubjectActivity.this,getString(R.string.op_success));
                        if (!mIsFav) {
                            mIsFav = true;
//                            mCollectTxt.setText(R.string.cancel_favorite);
                            mCollectTxt.setText(R.string.favorited);
                            ...
                            mIsFav = false;
                            mCollectTxt.setText(R.string.add_favorite);
                            mCollectTxt.setTextColor(getResources().getColor(R.color.tab_text_color_normal));
                            mCollect.setImageResource(R.drawable.course_detail_nocollect);
                        }
                    }else{
                        ToastUtil.showToast(SubjectActivity.this,getString(R.string.op_failed,
            ...
            case  CodeUtil.CmdCode.MsgTypeCode.MSG_GET_SUBJECT_DETAIL:{
                ...
                if(Utils.getRequestStatus(response) == 1) {
                    ...
                    if (content.getPhotos()!= null &&  !content.getPhotos().isEmpty()) {
                        Picasso.with(this).load(content.getPhotos()).error(R.drawable.banner_default).placeholder(R.drawable.banner_default).into(mSubjectImageView);
                    } else {
                    mSubjectImageView.setImageResource(R.drawable.banner_default);
                    }
                }else{
                    L.d(TAG,"resp give praise fail");
                }
 ...
 // 分享
    public void handleShare() {
        ...
        if(mSubjectTitle!=null){
            ...
            dialog.setShareInfo(mSubjectTitle.getText().toString(),
                    mSubjectDesc.getText().toString(), Urls.SHARE_ARTICLE_URL+mSubjectId);
        }else{
            dialog.setShareInfo("哼哼会",
                    "", Urls.SHARE_ARTICLE_URL+mSubjectId);
        }
