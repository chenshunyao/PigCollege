 ...
 */
@SuppressLint("NewApi")
public class BrowserFragment extends BaseFragment2 {
    ...
    public void onClick(View v) {
        ...
        case R.id.browser_system_browser:
            try {
                ...
                ToastUtil.showToast(getActivity(),"网页地址错误");
            }
            break;
        }
    }
    ...
    protected void onUrlLoading(WebView view, String url) {
        ...
        cookie.setCookie(url,
                HengHengHuiAppliation.getInstance().getProperty(AppConfig.CONF_COOKIE));
    }
    ...
    protected void onWebTitle(WebView view, String title) {
        if (aty != null && mWebView != null) { // 必须做判断，由于webview加载属于耗时操作，可能会本Activity已经关闭了才被调用
            ((BaseActivity2) aty).setActionBarTitle(mWebView.getTitle());
        }
    }
