...
public class VerifyCodeManager {
    ...
    public final static String MODIFY_PASSWD = "modifyPwd";
    ...
    public void getVerifyCode(String type) {
        // 获取验证码之前先判断手机号

        phone = phoneEdit.getText().toString().trim();

        if (TextUtils.isEmpty(phone)) {
            ToastUtil.showShort(mContext, R.string.tip_please_input_phone);
            return;
        } else if (phone.length() < 11) {
            ToastUtil.showShort(mContext, R.string.tip_phone_regex_not_right);
            return;
...
//				getVerifyCodeHandler);
        ...
        //获取验证码
        LoginManager.getVerifyCode(phone,type, TDevice.getIMEI());
    ...
    private void setButtonStatusOff() {
        int time =  recLen--;
        getVerifiCodeButton.setText("等待（"+String.valueOf(time)+"s）");
        //getVerifiCodeButton.setText(String.format(mContext.getResources().getString(R.string.count_down), recLen--));
        getVerifiCodeButton.setClickable(false);
        getVerifiCodeButton.setTextColor(Color.parseColor("#ffffff"));
        getVerifiCodeButton.setBackgroundColor(Color.parseColor("#11ab19"));
    }
