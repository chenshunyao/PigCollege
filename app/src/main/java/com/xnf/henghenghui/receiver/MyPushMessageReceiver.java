package com.xnf.henghenghui.receiver;
...
public class MyPushMessageReceiver extends PushMessageReceiver {
    /** TAG to Log */
    public static final String TAG = "csy_"+MyPushMessageReceiver.class
            .getSimpleName();
    public static final String LIVE_TAG = "liveConf";

    public static final String LIVE_MSG = "hasNewCommentInfo";
    ...
    public void onBind(Context context, int errorCode, String appid,
        ...
        if (errorCode == 0) {
            ...
            handleBind(context, appid, userId, channelId, requestId);
        }
    }

    private void handleBind(Context context, String appid,
                       String userId, String channelId, String requestId){
        ClientId = channelId;
    ...
    public void onMessage(Context context, String message,
        ...
        if (!TextUtils.isEmpty(message)) {
            ...
            try {
                ...
                if (!customJson.isNull("description")) {
                    JSONObject description = customJson.getJSONObject("description");
                    if (!description.isNull("type")) {
                        type = description.getInt("type");
                    }
                }
        ...
        String uid = LoginUserBean.getInstance().getLoginUserid();
        if(type == 0 || type == 1){//医生端被分配问题&客户端的问题被回答
            PreferencesWrapper.getInstance().setPreferenceIntValue("question"+uid, 1);
        }
        context.sendBroadcast(new Intent("com.hengheng.question.push"));
        //直播室消息推送
        if(mTag.equals(LIVE_TAG) && LIVE_MSG.equalsIgnoreCase(message)){
            context.sendBroadcast(new Intent("com.hengheng.liveroom.push"));
        }
        // Demo更新界面展示代码，应用请在这里加入自己的处理逻辑
//        updateContent(context, messageString);
    ...
    public void onSetTags(Context context, int errorCode,
        ...
        String responseString = "onSetTags errorCode=" + errorCode
                ...
                + " requestId=" + requestId;
        Log.d(TAG, responseString);
        if(errorCode == 0 && successTags.size() > 0){
            mTag = successTags.get(0);
        }
        // Demo更新界面展示代码，应用请在这里加入自己的处理逻辑
